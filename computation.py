...
def apply_color(smooth_iter, stripe, stripe_enabled, milnor_distance, brightness, ncycle, rgb_theta=(.0, .15, .25)):
    # brightness = overlay(brightness, stripe) * (1 - milnor_distance) + milnor_distance * brightness
    brightness = jnp.where(stripe_enabled, overlay(brightness, stripe) * (1 - milnor_distance) + milnor_distance * brightness, brightness)
